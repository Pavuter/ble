...
#else
#define BLE_UUID_NUS_TX_CHARACTERISTIC 0x1235                      /**< The UUID of the TX Characteristic. */
#define BLE_UUID_NUS_RX_CHARACTERISTIC 0x1236                      /**< The UUID of the RX Characteristic. */
 ...
 * @param[in] p_ble_evt Pointer to the received from BLE stack.
 */
...
static void on_write(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
    ...
    {
        p_nus->data_handler(p_nus, p_evt_write->data, p_evt_write->len); // 蓝牙接收数据处理函数 ble_nus_service...
    }
    else
    {
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    ble_uuid.type = p_nus->uuid_type; // 0x02
...
//
#if 0
signed int println(const char *fmt, ...)
{
