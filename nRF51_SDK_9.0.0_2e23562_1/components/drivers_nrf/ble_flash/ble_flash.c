...
uint32_t ble_flash_get_page_size(void)
{
    return NRF_FICR->CODEPAGESIZE;
}


uint32_t ble_flash_get_page_number(void)
{
...
void ble_flash_on_radio_active_evt(bool radio_active)
{
    ...
    //printf("radio_active = %d\r\n", radio_active);
}

bool ble_wait_radio_free(void){
  return m_radio_active;
}

void ble_flash_test(void)
{
	#define word_cnt 128
  uint32_t err_code = NRF_SUCCESS;

	uint32_t p_in[word_cnt], p_out[word_cnt];;
	uint16_t page_size  = ble_flash_get_page_size();
	uint16_t page_num   = ble_flash_get_page_number();
	uint16_t page_num_write = page_num - 1;
	
	uint8_t i = 0, flash_word_cnt;

	for(i = 0; i < word_cnt; i++)
	{
		p_in[i] = 0xaabbccdd+i;
	}
	LOG("start flash write.\r\n");
	err_code = ble_flash_page_write(page_num_write, &p_in[0], word_cnt);
	APP_ERROR_CHECK(err_code);
	LOG("flash write finished.\r\nread flash data:\r\n");
	err_code = ble_flash_page_read( page_num_write, p_out, &flash_word_cnt);
	APP_ERROR_CHECK(err_code);
