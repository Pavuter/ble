...
static int8_t channel_port_alloc(uint32_t pin, nrf_drv_gpiote_evt_handler_t handler, bool channel)
{
    ...
    start_idx = channel ? 0 : NUMBER_OF_GPIO_TE;
    end_idx   = channel ? NUMBER_OF_GPIO_TE : (NUMBER_OF_GPIO_TE + GPIOTE_CONFIG_NUM_OF_LOW_POWER_EVENTS);
    //critical section
                                  ...
                                  nrf_drv_gpiote_evt_handler_t evt_handler)
{
    ...
    else
    {
        ...
        if (channel != NO_CHANNELS)
        {
            ...
            {
                nrf_gpio_cfg_watcher(pin);  // 使能输入引脚追踪输出引脚
            }
            else
            {
            ...
            if (p_config->hi_accuracy)      // 使能高精度，需要更高的主频
            {
