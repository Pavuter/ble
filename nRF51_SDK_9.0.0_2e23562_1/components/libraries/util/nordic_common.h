...
#define __SYS_REMAIN_HEAP_SIZE_         app_get_heap_size()

// For debug 
/* #define DBG_LEVEL_HIGH									(1) */
/* #define DBG_LEVEL_MIDDLE								(1) */
/* #define DBG_LEVEL_LOW 									(1) */
/* #define DBG_LEVEL_NONE  								(1) */

#define DBG_LEVEL_NONE								(1)


#if DBG_LEVEL_HIGH
	#define LOG(fmt,...)   app_trace_log("[%d][L=%05d][ %-40s ] "fmt, \
                        __SYS_REMAIN_HEAP_SIZE_, __LINE__,__FUNCTION__, ##__VA_ARGS__);
	#define LOG_STRING		app_trace_dump
#elif DBG_LEVEL_MIDDLE
	#define LOG(fmt, ...) app_trace_log("[%04d] [%04d] "fmt, __SYS_REMAIN_HEAP_SIZE_, __LINE__, ##__VA_ARGS__)
	#define LOG_STRING		app_trace_dump
#elif DBG_LEVEL_LOW
  ...
  #define LOG_STRING
#else
	#param "Error debug.Please define debug level."
#endif
