...
#define BTN_ACTION_WORK_NORMAL    BTN_ID_WORK_NORMAL
 ...
 * @return A propagated error code.
 */
static uint32_t advertising_buttons_configure(void)
{
    uint32_t err_code;
...
uint32_t bsp_btn_ble_init(bsp_btn_ble_error_handler_t error_handler, bsp_event_t * p_startup_bsp_evt)
{
    ...
    #if 0
    err_code = bsp_event_to_button_action_assign(BTN_ID_WORK_NORMAL,
                                                 BTN_ACTION_WORK_NORMAL,
                                                 BSP_EVENT_KEY_0);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);
    #endif
    return err_code;
